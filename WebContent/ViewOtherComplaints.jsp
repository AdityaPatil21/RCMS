        ...
        <div class="row">
         <div class="col-md-12">
            <div class="title-wrap d-flex justify-content-center">
              <div class="title-box">
                <h4 class="title-a" style="color:#000;">View Other Complaints</h4>
              </div>
            </div>
          </div>
				...
				<div class="row">
                   ...
                   <table class="table table-bordered" align="center" style="background-color:#000;">
                    ...
                    /*  ResultSet rs=st.executeQuery("select * from streetlightcomplaint"); */
                     ResultSet rs=st.executeQuery("select othercomplaint.id,user.name,othercomplaint.image,othercomplaint.location,othercomplaint.pincode, othercomplaint.description,othercomplaint.status from othercomplaint inner join user on othercomplaint.email=user.email");
                     ...
                     if(rs.next() == false) {
                    	...
                    	do{	 
                    	 ...
                    	 <td><img alt="Profile Pic" class="img-circle" src="OtherPic.jsp?id=<%=rs.getInt(1) %>"  width="100px" height="80px"/></td>
                    	
                    	<td style="color:white"><%=rs.getString(4) %></td>
                    	 <td style="color:white"><%=rs.getString(5) %></td> 
                    	 <td style="color:white"><%=rs.getString(6) %></td>
                          
                    	 <th><a style="color:#2eca6a;" href="UpdateOtherStatusCon?id=<%=rs.getInt(1)%>&status=<%=rs.getString(7)%>"><%=rs.getString(7)%></a></th>
                    	<td><a style="color:red;" href="ReasonOther.jsp?id=<%=rs.getInt(1)%>"><b>Reason</b></a></td>
