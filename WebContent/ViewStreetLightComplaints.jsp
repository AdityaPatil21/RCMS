        ...
        <div class="row">
         <div class="col-md-12">
            <div class="title-wrap d-flex justify-content-center">
              <div class="title-box">
                <h4 class="title-a" style="color:#000;">View Street Light Complaints</h4>
              </div>
            </div>
          </div>
                    ...
                    /*  ResultSet rs=st.executeQuery("select * from streetlightcomplaint"); */
                     ResultSet rs=st.executeQuery("select streetlightcomplaint.id,user.name,streetlightcomplaint.image,streetlightcomplaint.location,streetlightcomplaint.pincode, streetlightcomplaint.description,streetlightcomplaint.status from streetlightcomplaint inner join user on streetlightcomplaint.email=user.email");
                     ...
                     if(rs.next() == false) {
                    	...
                    	do{	 
                    	 ...
                    	 <td><img alt="Profile Pic" class="img-circle" src="StreetLightPic.jsp?id=<%=rs.getInt(1) %>"  width="100px" height="80px"/></td>
                    	
                    	<td style="color:white"><%=rs.getString(4) %></td>
                    	 <td style="color:white"><%=rs.getString(5) %></td> 
                    	 <td style="color:white"><%=rs.getString(6) %></td>
                          
                    	 <th><a style="color:#2eca6a;" href="UpdateStreetlightStatusCon?id=<%=rs.getInt(1)%>&status=<%=rs.getString(7)%>"><%=rs.getString(7)%></a></th>
                    	  <td><a style="color:red;" href="ReasonStreetLight.jsp?id=<%=rs.getInt(1)%>"><b>Reason</b></a></td>
                    	 </TR>                    	 
                    <%count++; }while(rs.next());}%> 
